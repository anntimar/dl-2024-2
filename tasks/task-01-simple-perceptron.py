...
class Perceptron:
    def __init__(self, input_size, learning_rate=0.01, epochs=1000):
        self.weights = np.random.randn(input_size + 1) * 0.01  # Includes bias term
...
def generate_data(seed, samples, noise):
    ...
    y = np.where(y == 0, -1, 1)  # Converts labels to -1 and 1 for perceptron
    return X, y

def main():
    parser = argparse.ArgumentParser(description="Train a single-layer perceptron using NumPy.")
    parser.add_argument('--registration_number', type=int, help="Student's registration number (used as seed)")
    ...
    if args.registration_number is None:
        args.registration_number = 42  # Default seed value if not provided
